...
**RESULT (2026-07-09, headed A/B vs WS2, idle machine):** Select 21.2→6.1 (3.48x — beat the ~8–10 target), Partial update 40.6→21.4 (1.90x — hit ~20), Swap 41.7→23.8 (1.75x), Remove 25.3→16.9 (1.50x), Create 1K 38.9→32.3 (1.20x), Create 10K 409.6→347.7 (1.18x), Append 47.5→39.7 (1.20x), Replace 43.7→40.4, Clear flat; zero CPU regressions; CPU geomean **1.47x**; +1.5 KB gzip (compiler — WS6 diet list). (First A/B's Create-1K median of 70 ms was external interference: iterations 1–6 ran 32–33 ms, then an abrupt flat 70–73 ms cliff; a clean candidate-only re-run gave 15/15 iterations at 31.7–34.1 ms.) Landed: `_compileNode` AST→closure-tree compiler on the expression hot path — no per-eval scope object (`_collectKeys`/`Object.create` gone; identifiers resolve via raw ctx-chain walk + lazy $-specials/plugin/allow-list in exact interpreter precedence), `_exprCache` now stores compiled programs (fn + precompiled filter specs) keyed on full expr string. The planned fixed-shape **slot resolution was not needed** — chain-walk resolution alone beat the targets, so loop scopes keep full generality. Interpreter retained for statements and as differential reference (`_evaluateInterpreted`, tree-shaken from CDN bundle). Tests: `__tests__/evaluate-compiled.test.js` (206 cases: ~150-expr corpus + 400 seeded fuzz exprs through both engines, security pins incl. computed-key smuggling, Function/eval rewrap, arrow shadowing; validated by 3 injected-bug detection runs). Mandatory extra security review pass: done. Full Jest 2362 + e2e 475 green.
...
**RESULT (2026-07-09, headed A/B vs WS3 @ 280b57b):** Idle-machine probe: Clear rows 26.3→20.8 (**1.26x**), Replace 1.09x, Remove flat-to-slightly-better. The plan's ~13 ms Clear target is **unreachable from JS** — a microbenchmark with an empty clear handler shows the same ~20 ms floor; the remaining cost is native layout/paint, not framework work. Landed: `discard` flag on `_disposeTree` (element-only disposers tagged `_elOnly` skipped when the subtree is garbage; context/watcher/store cleanup still runs), `Range.deleteContents()`/`textContent=""` bulk clear in `_clearManagedClones`, full-replacement reconcile fast path (`anySurvives` early-exit check → bulk dispose + bulk append when no keys survive), devtools ctxId poke gated. ~+190 B gzip (WS6 diet list). **Benchmark-gate caveat (honest):** the decisive full A/B could not be run on an idle machine — two full runs were polluted by external load (proven: untouched Select path regressed identically with monotonic per-iteration drift; matched-session retry pulled all deltas to 0.94–1.00x within stddev, geomean 0.97x). No plausible code mechanism for regression (fast path early-exits; jfb uses no $-globals). A clean confirmation A/B runs when the machine idles; WS5 held until it lands. Tests: `__tests__/dispose-fast.test.js` (7: discard semantics, _elOnly skip, cleanup retention, bulk-clear correctness, fast-path equality); leak pins per plan; full Jest 2372/43 suites; e2e 478 passed (only the 13 pre-existing `main` failures). Also on this branch via rebase: **WS3 regression fix** 280b57b (`bind="$demo.count"` dead to `on:click="$demo.count++"` — compiled evaluate dropped the accidental `__collectKeysCache` keyless wake; fixed with explicit memoized `_stmtHasGlobalRoot` + keyless `$notify()` chain walk, +92 B gzip, 2 regression tests in plugin-system.test.js).
...
**RESULT (2026-07-09, headed A/B vs WS4 @ 5e3a880):** Run memory **6.2 → 4.3 MB (1.43x)**, Ready memory flat (1.1), Run+clear 1.8 → 1.9 (+0.1). Same-rig competitor context: crushes **Alpine 16.6**, beats **React-hooks 4.43**; **Vue 3.71 leads by ~0.6 MB**. A heap snapshot of the 1,000-row jfb page (aggregated by constructor, Playwright's own `InternalNode`/injected-script bytes filtered out) shows the remaining overhead is **diffuse and already lean** — the shared-Proxy handler is working (1 `JSProxy`/row, not 3), keys are interned, events delegated. The dominant reducible chunk is the **3 watcher closures/row + their 3 V8 scope Contexts** (`closure` + `system / Context / scope`), an artifact of NoJS's per-attribute-watcher model; closing it means a Vue-style compiled-binding rewrite (no per-binding watcher objects) — an architecture change deferred as out-of-scope for WS5's zero-breakage mandate. Vue's edge on this one param is **architectural**, not fat NoJS can trim safely.

Landed levers: (1) **shared Proxy handler** — one `_sharedHandler` for all contexts, per-context state reached via a `META` Symbol on the raw object (invisible to `Object.keys`/`_collectKeys`); lazy-cached `$watch`/`$notify`/`$set` closures on meta (stable identity). (2) **Interned `fn._keys`** — loop rows watching the same expression share one memoized root-key Set from evaluate.js (copy-on-write on union). (3) **Watcher chain-walk registration + data-driven disposal** — `_watchExpr` registers via direct `__listeners.add` up the context chain; non-gated elements record watchers as data (`fn._wctx` + `el.__watcherFns`) and `_disposeElement` re-walks to deregister, replacing one dispose closure per watcher. (4) **Document-level event delegation** — one listener per bubbling event type (`_DELEGATED_EVENTS` whitelist) dispatches to per-element `el.__on = { click: expr }` records; the bound context is hoisted once to `el.__onCtx` (identical for all events on an element), so no `[expr, ctx]` array per handler. Non-bubbling / modifier / lifecycle / gated events keep direct listeners. +~530 B gzip total (42,424 → 42,945; WS6 diet list).

**Run+clear +0.1 MB — investigated, benign.** A create→clear→GC heap snapshot shows **no per-row survivor**: `JSProxy`, watcher closures, and per-row scope Contexts are all reclaimed (disposal is clean, no leak). The +0.1 is retained `system / Map` **hidden-class descriptors** for WS5's new object shapes (meta record, `__on` record) — cached permanently by V8 and **count-independent** (same for 100 rows or 10,000), i.e. a fixed one-time cost, not a leak that scales. Tests: full Jest 2372/43 suites green; `directives-core` delegation test rewritten behaviorally (delegation has no per-element `removeEventListener` — assert a disposed-but-connected element's handler is dead); e2e chromium suite (cumulative verification for the delegation/disposal changes).
